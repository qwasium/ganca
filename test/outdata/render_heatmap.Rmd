...
plot function for heatmap using ggplot

Parameters
- heatmap_mat : matrix, win_x*win_y
- win_x       : numeric, window size x
- win_y       : numeric, window size y
- stim_img    : array, background image, default white blank
- leg_title   : string, legend title of ggplot
...
ggplot_heatmap <- function(heatmap_mat, win_x, win_y, stim_img = array(1, dim = c(win_y, win_x, 3)), draw_contour = TRUE, leg_title = ""){
  ...
  if (draw_contour == TRUE){
    heatmap_plot <- heatmap_plot + geom_contour(aes(z = heat), color = "white", linetype = "solid")
  }
